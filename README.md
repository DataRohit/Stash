...
- **Schema** lives in `convex/schema.ts` (`organizations` for per-org description/tags, `members` mirroring org members and pending invitations, and `projects` + `projectAccess` for org projects and per-member access grants). Backend functions go in `convex/`. Organization icons use Clerk's org logo, but **project icons are stored in Convex file storage** (with a DiceBear default).
