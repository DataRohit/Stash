...
export function Navbar() {
  ...
  return (
    ...
    >
      <div
        className={cn(
          "glass mx-auto flex h-14 max-w-7xl items-center justify-between rounded-lg px-3 sm:px-4",
        ...
        <nav className="hidden items-center gap-7 md:flex">
          {navLinks.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className="rounded-sm px-2 py-1.5 font-medium font-mono text-muted-foreground text-xs uppercase tracking-wider transition-colors hover:bg-foreground/[0.05] hover:text-foreground"
            >
              {link.label}
            </a>
          ))}
